...
| [`prettifier/`](prettifier) | Chrome & Firefox | Beautifies JSON and Markdown, runs a structural comparison between two versions of either — JSON diffed by property path, Markdown diffed section by section — and parses a URL's query string into editable, copyable Name → Value rows, with no host permissions at all. |
| [`storage-inspector/`](storage-inspector) | Chrome & Firefox | Views, edits and deletes localStorage and sessionStorage entries for the active tab's page, with export to JSON — access granted only to the tab you click on. |
